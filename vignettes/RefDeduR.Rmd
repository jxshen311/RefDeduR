...
We further look for potential duplicates according to pairwise string similarity between records within the same partitioned group for columns `"title_norm"` and `"abstract_norm"`. 
...
Following a pipeline similar to that at step 1, we first calculate string similarity. Because we partition the dataset, results are now stored in lists (as compared to data frames at step 1).
```{r}
c(ls_b3, ls_b3_simi) %<-% simi_ptn_pair(b3, partition_by = "first_two_letters_first_author_last_name")
# Computing time estimation:  for this data frame ( rows) on a Macbook Pro (Apple M1 Pro chip basic model, memory: 16 GB).
```
